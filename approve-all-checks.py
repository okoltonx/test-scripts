...
def main():
    ...
    parser.add_argument("--token", help="GitHub token (overshadowuje wartość w kodzie DEFAULT_TOKEN)")
    args = parser.parse_args()

    token = args.token or DEFAULT_TOKEN
    if not token or token == "<YOUR_GITHUB_TOKEN_HERE>":
        print("Podaj ważny token GitHub w DEFAULT_TOKEN lub przez parametr --token.", file=sys.stderr)
        sys.exit(1)
    ...
    for ctx in contexts:
        set_status(token, owner, repo, sha, ctx)
